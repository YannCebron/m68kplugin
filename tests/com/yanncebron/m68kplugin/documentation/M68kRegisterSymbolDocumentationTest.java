...
public class M68kRegisterSymbolDocumentationTest extends BasePlatformTestCase {
  ...
  public void testRegistersWithoutReferenceDocs() {
    Set<M68kRegister> withoutDoc = new HashSet<>();
    for (M68kRegister register : M68kRegister.values()) {
      String registerDoc = new M68kRegisterDocsGenerator(register).getDocumentation(true);
  ...
  public void testDataRegister() {
    doTest(" move.l d<caret>0,d1", "D0", "<style>table { white-space: nowrap; } blockquote { padding-left: 10px; padding-right: 10px; padding-bottom: 5px; }</style><div class='definition'><pre><b>D0</b></pre></div><table class='sections'><tr><td valign='top' class='section'><p>CPU:</td><td valign='top'>MC68000 Family</td></table><div class='content'><h2>Data Registers (<code>D0</code>-<code>D7</code>)</h2>\n" +
      ...
      "<br><br><small><em>From MOTOROLA M68000 FAMILY Programmer's reference manual. Copyright 1992 by Motorola Inc./NXP. Adapted with permission.</em></small></div>");
  }

  public void testSfcRegister() {
    doTest(" movec S<caret>FC,d0", "SFC", "<style>table { white-space: nowrap; } blockquote { padding-left: 10px; padding-right: 10px; padding-bottom: 5px; }</style><div class='definition'><pre><b>SFC</b></pre></div><table class='sections'><tr><td valign='top' class='section'><p>CPU:</td><td valign='top'>MC68010+</td></table><div class='content'><h2>Alternate Function Code Registers (<code>SFC</code> and <code>DFC</code>)</h2>\n" +
  ...
  public void testRemapToAddressRegister() {
    ...
    for (M68kRegister register : remapRegisters) {
      ...
      assertTrue("failed for " + register + ": " + documentation, documentation.contains("<h2>Address registers (<code>A0</code>-<code>A7</code>)</h2>"));
    }
  }
