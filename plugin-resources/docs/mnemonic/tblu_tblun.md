...
## Description
The `TBLU` and `TBLUN` instructions allow the efficient use of piecewise linear,
...
variable, Y, as a function of X. In general, the independent variable, located in the 
low-order word of Dx, consists of an 8-bit integer part and an 8-bit fractional part. An
...
The unsigned and unrounded table results will be zero-extended instead of sign-extended.
...
2. Interpolation resolution is limited to 1/256, the distance between consecutive table
   ...
   fractional number in the range 0 &le; X &le; 255. On the contrary, X should be considered
